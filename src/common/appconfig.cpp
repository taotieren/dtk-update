    ...
    , d(new Private)
{
    ...
    d->cfg = DConfig::create(QStringLiteral("org.deepin.dtk-update"),
                             QStringLiteral("org.deepin.dtk-update"),
                             QString(), this);
    if (!d->cfg) {
...
void AppConfig::loadConfigFile()
{
    ...
    if (d->effectiveBackendId.isEmpty()) {
        ...
        QString fromDc = d->cfg ? d->cfg->value("preferredBackend", QString()).toString()
                                 : QString();
...
QString AppConfig::showConfig() const
{
    ...
    QString out;
    out += QStringLiteral("# Effective configuration (merged: backend.conf > DConfig > distro preset)\n");
    out += QStringLiteral("PreferredBackend = ") + d->effectiveBackendId + QLatin1Char('\n');
    out += QStringLiteral("CheckIntervalMinutes = ") + QString::number(checkIntervalMinutes()) + QLatin1Char('\n');
    out += QStringLiteral("NoInstallRecommends = ") + (noInstallRecommends() ? "true" : "false") + QLatin1Char('\n');
    out += QStringLiteral("AutoRemoveOrphans = ") + (autoRemoveOrphans() ? "true" : "false") + QLatin1Char('\n');
    out += QStringLiteral("AutoCleanCache = ") + (autoCleanCache() ? "true" : "false") + QLatin1Char('\n');
    out += QStringLiteral("ShowSecurityAdvisory = ") + (showSecurityAdvisory() ? "true" : "false") + QLatin1Char('\n');
    out += QStringLiteral("FetchUpstreamAdvisories = ") + (fetchUpstreamAdvisories() ? "true" : "false") + QLatin1Char('\n');
    out += QStringLiteral("DistroFamily = ") + DistroProbe::familyName(d->family) + QLatin1Char('\n');
    return out;
}
