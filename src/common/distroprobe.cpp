...
DistroProbe::Family DistroProbe::detectFamily() { return detectFamilyFrom("/etc/os-release"); }
QString DistroProbe::detectId() { return detectIdFrom("/etc/os-release"); }
...
DistroProbe::Family DistroProbe::detectFamilyFrom(const QString &osReleasePath)
{
    ...
    for (const QString &like : idLike) {
        if (like == "debian") return Family::Debian;
        if (like == "fedora") return Family::Fedora;
        if (like == "archlinux") return Family::Arch;
        if (like == "suse") return Family::Suse;
    }
