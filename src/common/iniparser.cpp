    ...
    bool IniParser::parse(const QString& text, QString* error)
    {
        ...
        while (stream.readLineInto(&line))
        {
            ...
            if (raw.startsWith(QLatin1Char('[')) && raw.endsWith(QLatin1Char(']')))
            {
                ...
                {
                    if (error)
                        *error = QStringLiteral("empty section header at line %1").arg(lineNo);
                    return false;
                }
            ...
            {
                if (error)
                    *error = QStringLiteral("missing '=' in line %1").arg(lineNo);
                return false;
            }
            ...
            {
                if (error)
                    *error = QStringLiteral("empty key in line %1").arg(lineNo);
                return false;
            }
            if (curSection.isEmpty())
