...
namespace DtkUpdate
{
    ...
    DtkUpdatePlugin::DtkUpdatePlugin(QObject* parent) : QObject(parent)
    {
    ...
    void DtkUpdatePlugin::buildBackend()
    {
        ...
        connect(m_monitor, &UpdateMonitor::securityPrompt, this,
                [this](const QString& sev, const QList<SecurityAdvisor::Advisory>& advs,
                       const PreCheckReport& pre) { showSecurityConfirm(sev, advs, pre); });
                                              ...
                                              const PreCheckReport& pre)
    {
        ...
        dlg.addButton(tr("Cancel"), true,
                      DDialog::ButtonNormal); // 默认聚焦：取消（不主动替用户决定）
