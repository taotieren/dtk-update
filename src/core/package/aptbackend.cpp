...
bool AptBackend::isAvailable() const
{
    ...
    QString out, err;
    if (!runQuery(QStringList{QStringLiteral("list"), QStringLiteral("--upgradable")}, out, err))
        return false;
    return true;
}
...
bool AptBackend::runPrivileged(const QStringList &aptArgs, QString &output, QString &error) const
{
...
bool AptBackend::fetchUpgradable(PackageList &out, QString &error)
{
    ...
    QString raw;
    if (!runQuery({QStringLiteral("apt"), QStringLiteral("list"),
                   QStringLiteral("--upgradable")}, raw, error)) {
        return false;
    }
...
bool AptBackend::listInstalled(PackageList &out, const QString &filter, QString &error)
{
    ...
    QString raw;
    if (!runQuery({QStringLiteral("dpkg-query"), QStringLiteral("-W"),
                   QStringLiteral("-f"), format}, raw, error)) {
        return false;
    }
...
{
    // 干跑解析依赖（不实际安装）
    return runQuery({QStringLiteral("apt-get"), QStringLiteral("install"),
                     QStringLiteral("-s"), pkg}, resolution, error);
}
...
bool AptBackend::listResidualPackages(PackageList &out, QString &error)
{
    ...
    while (stream.readLineInto(&line)) {
        ...
        if (parts.size() > 2) info.currentVersion = parts.at(2);
        if (parts.size() > 3) info.architecture = parts.at(3);
        if (parts.size() > 4) info.section = parts.at(4);
        out.append(info);
    }
    return true;
}
...
QVariantMap AptBackend::backendOptions() const
{
    QVariantMap opts;
    if (m_config) {
        opts.insert(QStringLiteral("noInstallRecommends"),
                    m_config->noInstallRecommends());
...
bool AptBackend::autoremove(QString &error)
{
    ...
    runPrivilegedAsync([this](QString &out, QString &err) {
        return runPrivileged({QStringLiteral("autoremove"), QStringLiteral("-y"),
                              QStringLiteral("--purge")}, out, err);
    });
    return true;
}
...
void AptBackend::collectConfigFiles(const QDir &dir, const QStringList &suffixes,
