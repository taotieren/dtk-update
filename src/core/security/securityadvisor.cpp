...
QString offlineSeverity(const QString &pkg)
{
    static const QStringList critical = {
        ...
        QStringLiteral("sudo"), QStringLiteral("polkitd"), QStringLiteral("policykit"),
        QStringLiteral("dbus"), QStringLiteral("shadow"), QStringLiteral("passwd"),
    };
    static const QStringList high = {
        QStringLiteral("apt"), QStringLiteral("dpkg"), QStringLiteral("bash"),
        QStringLiteral("coreutils"), QStringLiteral("glibc"), QStringLiteral("gnutls"),
        QStringLiteral("libxml2"), QStringLiteral("curl"), QStringLiteral("wget"),
        QStringLiteral("firefox"), QStringLiteral("chromium"), QStringLiteral("openssh"),
        QStringLiteral("ssh"), QStringLiteral("cryptsetup"), QStringLiteral("dde-daemon"),
        QStringLiteral("deepin-daemon"),
    };
    for (const auto &c : critical)
        if (pkg.startsWith(c)) return QStringLiteral("critical");
    for (const auto &h : high)
        if (pkg.startsWith(h)) return QStringLiteral("high");
...
SecurityAdvisor::SecurityAdvisor(QObject *parent) : QObject(parent)
{
    ...
    if (!m_available)
        qCInfo(dtkUpdateCore) << "deepin security center D-Bus unavailable, fallback to offline heuristic";
}
...
QString SecurityAdvisor::upstreamFeedUrl(const QString &distroId)
{
    ...
    static const QMap<QString, QString> feeds = {
        ...
        // Fedora 安全公告
        { QStringLiteral("fedora"), QStringLiteral("https://www.openwall.com/lists/oss-security/") },
    };
    return feeds.value(distroId.toLower());
}

QList<SecurityAdvisor::Advisory> SecurityAdvisor::fetchUpstreamFor(
    const QString &distroId, const QStringList &packages)
{
...
bool SecurityAdvisor::fetchAdvisories(const QStringList &packages, QList<Advisory> &out)
{
    ...
    for (const auto &pkg : packages) {
        ...
        a.description = tr("This package is security-sensitive. Review the changelog before updating.");
