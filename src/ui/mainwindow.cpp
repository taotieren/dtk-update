...
MainWindow::MainWindow(PackageBackend *backend,
                       AppConfig *config,
    ...
    , m_advisor(advisor)
{
    ...
    connect(m_monitor, &UpdateMonitor::securityPrompt,
            this, &MainWindow::onSecurityPrompt);
    connect(m_monitor, &UpdateMonitor::postCheck, this, &MainWindow::onPostCheck);
    connect(m_monitor, &UpdateMonitor::upgradeProgress,
            this, &MainWindow::onUpgradeProgress);
    connect(m_monitor, &UpdateMonitor::upgradeFinished,
...
void MainWindow::buildUI()
{
    ...
    connect(d->updateList->selectionModel(),
...
void MainWindow::onStateChanged(UpdateMonitor::State state)
{
    switch (state) {
        ...
        case UpdateMonitor::State::Error:
            ...
            d->updateBtn->setEnabled(false);
            if (d->spinner) { d->spinner->stop(); d->spinner->deleteLater(); d->spinner = nullptr; }
            break;
                              ...
                              const PreCheckReport &pre)
{
    ...
    for (const QString &s : pre.servicesNeedRestart)
        preText += QStringLiteral("<br>• %1: %2")
                       .arg(tr("Service needs restart"), s.toHtmlEscaped());
    for (const QString &c : pre.configFilesToReview)
        preText += QStringLiteral("<br>• %1: %2")
                       .arg(tr("Config file to review"), c.toHtmlEscaped());
    for (const QString &u : pre.failedUnits)
        preText += QStringLiteral("<br>• %1: %2")
                       .arg(tr("Failed service unit"), u.toHtmlEscaped());
    if (!preText.isEmpty())
    ...
    const int updateId = dlg.addButton(QStringLiteral("Update"), true, DDialog::ButtonRecommend);
...
void MainWindow::showPostCheckHint(const PostCheckReport &report)
{
    ...
    if (report.rebootRequired)
        text += QStringLiteral("<br>• <b>%1</b>")
                    .arg(tr("A system reboot is recommended (kernel or base library updated)."));
    for (const QString &s : report.servicesNeedRestart)
        text += QStringLiteral("<br>• %1: %2").arg(tr("Service needs restart"), s.toHtmlEscaped());
    for (const QString &c : report.configFilesToReview)
        text += QStringLiteral("<br>• %1: %2").arg(tr("Config file to review"), c.toHtmlEscaped());
    for (const QString &u : report.failedUnits)
        text += QStringLiteral("<br>• %1: %2").arg(tr("Failed service unit"), u.toHtmlEscaped());
    for (const QString &p : report.residualPackages)
        text += QStringLiteral("<br>• %1: %2").arg(tr("Residual / orphan package"), p.toHtmlEscaped());
    ...
    if (!report.error.isEmpty())
        text += QStringLiteral("<br><span style=\"color:#888\">%1</span>").arg(report.error.toHtmlEscaped());
    dlg.setMessage(text);
