...
## 长任务后多 Agent 安全审查（硬纪律，防幻觉 / 自欺）

连续多轮、跨多文件的功能开发或重构收尾时，**不得仅由主 agent 自审即定稿**——长上下文下
极易产生"看起来合理、实际未实现 / 与约定冲突"的自欺结论。必须在提交前拉起**三个相互独立
的审查 agent** 做交叉验证：

1. **触发条件**：本轮新增 / 修改源码超过约 200 行，或跨 3 个以上文件，或涉及 `src/core`
   抽象 / 后端注册 / 提权 / 异步写等硬性约束时，强制触发。
2. **三个 agent 的分工（互不共享上下文，独立给出结论）**：
   - **Agent A（规范符合性）**：逐条对照本 AGENTS.md 的硬性约束、已踩过的坑、字符串/翻译/
     文档联动纪律，核查是否违规或漂移。
   - **Agent B（实现正确性）**：基于源码静态分析，验证"声称实现的能力是否真的有对应代码"，
     重点排查死代码、虚设开关、声明/实现不一致、异步/内存/提权陷阱。
   - **Agent C（安全与依赖面）**：审查提权路径（pkexec 而非进程内 sudo）、D-Bus/网络异步
     是否阻塞、输入解析（LC_ALL=C、tab/退出码）、第三方依赖与权限边界，确认无引入风险。
3. **交叉对质**：主 agent 汇总三份报告，对**相互矛盾**或**指向同一风险**的结论优先处理；
   任一 agent 标记"未实现 / 可疑"的项，必须回到源码核实并修复或澄清，禁止以"应该没问题"
   搪塞。三份审查结论与最终处置（修复 commit 号 / 已澄清项）须记录在当日工作记忆与提交说明。
4. **闸门**：多 agent 审查发现的高危项未闭环前，**不提交、不推送**。
